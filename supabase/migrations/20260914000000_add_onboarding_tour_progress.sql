...
create index onboarding_tour_progress_school_user_idx on public.onboarding_tour_progress(school_id, user_id);
alter table public.onboarding_tour_progress enable row level security;
create policy onboarding_tour_progress_backend_only on public.onboarding_tour_progress for all to app_backend using (true) with check (true);
