...
const blockAccents: Accent[] = [
  { card: 'border-blue-200 bg-blue-50/70', border: 'border-blue-200', panel: 'bg-blue-50 text-blue-950', text: 'text-blue-700', badge: 'bg-blue-100 text-blue-700 ring-blue-200', progressColor: '#2563eb', gradient: 'linear-gradient(90deg, #60a5fa 0%, #2563eb 55%, #1d4ed8 100%)' },
  { card: 'border-emerald-200 bg-emerald-50/70', border: 'border-emerald-200', panel: 'bg-emerald-50 text-emerald-950', text: 'text-emerald-700', badge: 'bg-emerald-100 text-emerald-700 ring-emerald-200', progressColor: '#059669', gradient: 'linear-gradient(90deg, #34d399 0%, #059669 55%, #047857 100%)' },
  { card: 'border-amber-200 bg-amber-50/70', border: 'border-amber-200', panel: 'bg-amber-50 text-amber-950', text: 'text-amber-700', badge: 'bg-amber-100 text-amber-700 ring-amber-200', progressColor: '#f59e0b', gradient: 'linear-gradient(90deg, #fcd34d 0%, #f59e0b 55%, #d97706 100%)' },
  { card: 'border-violet-200 bg-violet-50/70', border: 'border-violet-200', panel: 'bg-violet-50 text-violet-950', text: 'text-violet-700', badge: 'bg-violet-100 text-violet-700 ring-violet-200', progressColor: '#7c3aed', gradient: 'linear-gradient(90deg, #a78bfa 0%, #7c3aed 55%, #6d28d9 100%)' },
]

const blockLongNames: Record<string, string> = {
...
export function ActivityInfoModal({ activity, onClose, onEdit, onEvaluate }: {
  ...
  const accent = blockAccents[Math.max(0, competencyBlocks.findIndex((item) => item.id === block.id))] ?? blockAccents[0]
  const resources = activity.resources ?? []
  const activityDetails = [
    { icon: <Target className="size-4" />, label: 'Valor', value: `${activity.maxScore} pts` },
    { icon: <CalendarDays className="size-4" />, label: 'Fecha', value: formatDate(activity.date) },
    { icon: <Tags className="size-4" />, label: 'Técnica', value: techniqueLabel(activity.evaluationTechnique) },
    ...
    { icon: <Clock3 className="size-4" />, label: 'Momento', value: momentLabel(activity.planningMoment) },
  ]
    ...
    <Modal title="Detalle de la actividad" onClose={onClose} hideHeader className="max-h-[92vh] max-w-6xl rounded-2xl">
      ...
      <div className="space-y-4 bg-muted/10 p-5">
        <section className={cn('relative overflow-hidden rounded-xl border p-4 shadow-sm', accent.card, accent.border)}>
          <div className="relative z-10 flex flex-wrap items-start justify-between gap-3"><div><h4 className={cn('text-xl font-black', accent.text)}>{activity.name || 'Actividad sin nombre'}</h4><p className="mt-1 text-sm text-muted-foreground">{blockLongNames[block.id] ?? block.name}</p></div><Badge className={cn(accent.badge)}>{block.shortName}</Badge></div>
        ...
        <div className="grid items-start gap-4 lg:grid-cols-[minmax(0,1.45fr)_minmax(16rem,0.55fr)]">
          <section className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
            ...
            <div className="whitespace-pre-wrap p-4 text-sm leading-7 text-foreground">{plainActivityText(activity.description ?? '') || 'No hay una descripción registrada para esta actividad.'}</div>
          </section>
          <section className={cn('overflow-hidden rounded-xl border bg-card shadow-sm', accent.border)}>
            ...
            {resources.length ? <div className="flex flex-wrap gap-2 p-4">{resources.map((resource) => <span key={resource} className={cn('inline-flex items-center gap-1.5 rounded-lg border bg-card px-2.5 py-1.5 text-xs font-bold shadow-sm', accent.border, accent.text)}>{resourceIcon(resource)}{resource}</span>)}</div> : <div className="flex items-center gap-2 p-4 text-xs text-muted-foreground"><Box className="size-4" />No se registraron recursos.</div>}
          </section>
        </div>
...
function ReadOnlyInstrument({ type, fields, maxScore, accent }: { type?: string; fields: Record<string, string>; maxScore: number; accent: Accent }) {
  ...
  if (type === 'rubrica') {
    ...
    return <div className="space-y-2"><InstrumentTable><thead><tr><th className="border border-border bg-slate-50 px-3 py-2">Criterios</th>{levels.map((level, index) => { const visual = rubricLevelVisual(index, levels.length); return <th key={level} className="border px-3 py-2 text-center" style={{ backgroundColor: visual.background, borderColor: visual.border, color: visual.foreground }}><span className="block font-black">{fields[fieldKey(type, 'level-name', level)] || `Nivel ${level}`}</span><span className="text-[10px] font-bold opacity-80">{fields[fieldKey(type, 'level-points', level)] || level} pts</span></th> })}<th className="border border-border bg-slate-50 px-3 py-2 text-center">Valor</th></tr></thead><tbody>{indexes.map((index) => <tr key={index}><td className="border border-border px-3 py-3 font-bold">{fields[fieldKey(type, 'criterion', index)]}</td>{levels.map((level) => <td key={level} className="border border-border px-3 py-3 leading-5 text-muted-foreground">{fields[fieldKey(type, 'descriptor', index, level)] || '—'}</td>)}<td className="border border-border px-3 py-3 text-center font-bold">{fields[fieldKey(type, 'points', index)] || '—'} pts</td></tr>)}</tbody></InstrumentTable><p className="text-right text-xs font-black text-primary">Puntuación máxima: {maxScore} pts</p></div>
  }

  if (type === 'lista-cotejo') {
    const hasNoApply = fields['lista-cotejo:meta:noApply'] === 'true'
    const options = [{ label: fields['lista-cotejo:meta:yesLabel'] || 'Sí', tone: 'positive' }, { label: fields['lista-cotejo:meta:noLabel'] || 'No', tone: 'negative' }, ...(hasNoApply ? [{ label: fields['lista-cotejo:meta:naLabel'] || 'No aplica', tone: 'neutral' }] : [])]
    return <div className="space-y-3"><InstrumentTable><thead><tr><th className={cn('border px-3 py-3', accent.card, accent.border, accent.text)}>Criterios</th>{options.map((option) => <th key={option.label} className={cn('border px-3 py-3 text-center', option.tone === 'positive' ? 'border-emerald-300 bg-emerald-100 text-emerald-800' : option.tone === 'negative' ? 'border-red-300 bg-red-100 text-red-700' : 'border-amber-300 bg-amber-100 text-amber-800')}>{option.label}</th>)}<th className={cn('border px-3 py-3 text-center', accent.card, accent.border, accent.text)}>Valor</th></tr></thead><tbody>{indexes.map((index) => <tr key={index}><td className="border border-border bg-card px-3 py-3 font-bold">{fields[fieldKey(type, 'criterion', index)] || `Criterio ${index + 1}`}</td>{options.map((option) => <td key={option.label} className={cn('border text-center', option.tone === 'positive' ? 'border-emerald-200 bg-emerald-50/55' : option.tone === 'negative' ? 'border-red-200 bg-red-50/55' : 'border-amber-200 bg-amber-50/55')}><span className={cn('mx-auto block size-4 rounded-full border-2 bg-card', option.tone === 'positive' ? 'border-emerald-500' : option.tone === 'negative' ? 'border-red-400' : 'border-amber-500')} /></td>)}<td className={cn('border px-3 py-3 text-center font-black', accent.card, accent.border, accent.text)}>{fields[fieldKey(type, 'points', index)] || 0} pts</td></tr>)}</tbody></InstrumentTable><p className={cn('text-right text-sm font-black', accent.text)}>Puntuación máxima: {maxScore} pts</p></div>
  }
  ...
  if (type === 'escala') {
    ...
    return <div className="space-y-3"><InstrumentTable><thead><tr><th className={cn('border px-3 py-3', accent.card, accent.border, accent.text)}>Indicadores</th>{levels.map((level, index) => { const visual = scaleLevelVisual(accent, index, levels.length); return <th key={level} className="border px-3 py-3 text-center" style={{ backgroundColor: visual.background, borderColor: visual.border, color: visual.foreground }}><span className="block font-black">{fields[fieldKey(type, 'level-name', level)] || `Nivel ${level}`}</span><span className="text-[10px] font-medium opacity-75">{fields[fieldKey(type, 'level-points', level)] || level} pts</span></th> })}{hasNoApply ? <th className="border border-slate-300 bg-slate-100 px-3 py-3 text-center text-slate-600">No aplica<span className="block text-[10px]">N/A</span></th> : null}<th className={cn('border px-3 py-3 text-center', accent.card, accent.border, accent.text)}>Máximo</th></tr></thead><tbody>{indexes.map((index) => <tr key={index}><td className="border border-border bg-card px-3 py-3 font-bold">{fields[fieldKey(type, 'criterion', index)] || `Indicador ${index + 1}`}</td>{levels.map((level, levelIndex) => { const visual = scaleLevelVisual(accent, levelIndex, levels.length); return <td key={level} className="border bg-card text-center" style={{ borderColor: visual.border }}><span className="mx-auto block size-4 rounded-full border-2 bg-card" style={{ borderColor: visual.border }} /></td> })}{hasNoApply ? <td className="border border-slate-200 bg-slate-50 text-center"><span className="mx-auto block size-4 rounded-full border-2 border-slate-300 bg-card" /></td> : null}<td className={cn('border px-3 py-3 text-center font-black', accent.card, accent.border, accent.text)}>{fields[fieldKey(type, 'points', index)] || 0} pts</td></tr>)}</tbody></InstrumentTable><p className={cn('text-right text-sm font-black', accent.text)}>Puntuación máxima: {maxScore} pts</p></div>
  }
  ...
  const totalWeight = indexes.reduce((sum, index) => sum + Number(fields[fieldKey(type, 'weight', index)] || 0), 0)
  return <div className="space-y-3"><InstrumentTable><thead><tr><th className={cn('border px-3 py-2', accent.card, accent.border)}>Criterio</th><th className={cn('border px-3 py-2', accent.card, accent.border)}>Indicador observable</th><th className={cn('border px-3 py-2 text-center', accent.card, accent.border)}>Ponderación</th><th className={cn('border px-3 py-2 text-center', accent.card, accent.border)}>Valor máximo</th><th className="border border-emerald-200 bg-emerald-50 px-3 py-2 text-center text-emerald-700">{labels.yes}<span className="block text-[9px]">100 %</span></th>{hasPartial ? <th className="border border-amber-200 bg-amber-50 px-3 py-2 text-center text-amber-700">{labels.partial}<span className="block text-[9px]">{fields['lista-ponderada:meta:partialValue'] || 50} %</span></th> : null}<th className="border border-red-200 bg-red-50 px-3 py-2 text-center text-red-700">{labels.no}<span className="block text-[9px]">0 %</span></th></tr></thead><tbody>{indexes.map((index) => { const weight = Number(fields[fieldKey(type, 'weight', index)] || 0); return <tr key={index}><td className="border border-border bg-card px-3 py-3 font-bold">{fields[fieldKey(type, 'criterion', index)] || `Criterio ${index + 1}`}</td><td className="border border-border bg-card px-3 py-3 text-muted-foreground">{fields[fieldKey(type, 'indicator', index)] || '—'}</td><td className="border border-border bg-card px-3 py-3 text-center font-black">{formatNumber(weight)} %</td><td className={cn('border px-3 py-3 text-center font-black', accent.card, accent.border, accent.text)}>{formatNumber(weight / 100 * maxScore)} pts</td><td className="border border-emerald-200 bg-emerald-50/40 text-center"><span className="mx-auto block size-4 rounded-full border-2 border-emerald-500 bg-card" /></td>{hasPartial ? <td className="border border-amber-200 bg-amber-50/40 text-center"><span className="mx-auto block size-4 rounded-full border-2 border-amber-500 bg-card" /></td> : null}<td className="border border-red-200 bg-red-50/40 text-center"><span className="mx-auto block size-4 rounded-full border-2 border-red-400 bg-card" /></td></tr> })}</tbody></InstrumentTable><div className={cn('flex justify-between rounded-xl border px-4 py-3 text-sm font-black', accent.card, accent.border)}><span className={Math.abs(totalWeight - 100) < .001 ? 'text-emerald-700' : 'text-destructive'}>Ponderación: {formatNumber(totalWeight)}/100 %</span><span className={accent.text}>Puntuación máxima: {formatNumber(maxScore)} pts</span></div></div>
}

function InstrumentTable({ children }: { children: React.ReactNode }) { return <div className="overflow-x-auto rounded-xl border border-border bg-card"><table className="w-full min-w-[44rem] border-collapse text-xs">{children}</table></div> }
function fieldKey(type: string, field: string, index: number, level?: number) { return level === undefined ? `${type}:${field}:${index}` : `${type}:${field}:${index}:${level}` }
...
function scaleLevelVisual(accent: Accent, index: number, count: number) { const ratio = count <= 1 ? 0 : index / (count - 1); const bg = Math.round(20 - ratio * 13); const border = Math.round(52 - ratio * 22); return { background: `color-mix(in srgb, ${accent.progressColor} ${bg}%, white)`, border: `color-mix(in srgb, ${accent.progressColor} ${border}%, white)`, foreground: `color-mix(in srgb, ${accent.progressColor} 72%, #0f172a)` } }
function formatNumber(value: number) { return Number.isInteger(value) ? String(value) : Number(value.toFixed(2)).toString() }
function instrumentTitle(value?: string) { return ({ rubrica: 'Rúbrica de evaluación', 'lista-cotejo': 'Lista de cotejo', escala: 'Escala estimativa', 'lista-ponderada': 'Lista ponderada' } as Record<string, string>)[value ?? ''] ?? value ?? 'Sin instrumento' }
function momentLabel(value?: string) { return ({ inicio: 'Inicio', desarrollo: 'Desarrollo', cierre: 'Cierre' } as Record<string, string>)[value ?? ''] ?? 'Sin definir' }
function techniqueLabel(value?: string) { if (!value) return 'Sin técnica'; return value.split('-').map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join(' ') }
function formatDate(value?: string | null) { if (!value) return 'Sin fecha'; const date = new Date(value); return Number.isNaN(date.getTime()) ? value : date.toLocaleDateString('es-DO', { day: '2-digit', month: '2-digit', year: 'numeric' }) }
