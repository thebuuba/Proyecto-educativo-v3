...
export function PlanningDocumentView({ entry, onClose }: PlanningDocumentViewProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-primary/35 px-3 py-4 backdrop-blur-[2px] sm:px-4 sm:py-6">
      <div ref={dialogRef} className="flex max-h-[94vh] w-full max-w-7xl flex-col overflow-hidden rounded-3xl bg-background shadow-2xl">
        <div className="flex shrink-0 flex-col gap-3 border-b border-border bg-card px-4 py-3 lg:flex-row lg:items-center lg:justify-between lg:px-5">
          <div className="flex min-w-0 items-center gap-3">
            <SemanticIcon icon={FileText} tone="warning" className="size-10 rounded-xl" iconClassName="size-4" />
            <div className="min-w-0">
              <p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-muted-foreground">Vista documental</p>
              <h3 className="mt-0.5 truncate text-base font-extrabold text-foreground">{entry.title}</h3>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <StatusBadge tone="warning">{planningTypeLabels[entry.planningType ?? 'DAILY']}</StatusBadge>
            ...
            <Button variant="ghost" size="icon" aria-label="Cerrar vista documento" onClick={onClose}><X className="size-5" /></Button>
          </div>
        </div>

        <div className="overflow-y-auto bg-muted/35 p-3 sm:p-5">
          <article className="mx-auto max-w-[1180px] overflow-hidden rounded-2xl bg-card shadow-sm">
            <header className="flex flex-col gap-4 border-b border-border px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
              <div className="flex items-center gap-3">
                <SemanticIcon icon={FileText} tone="warning" className="size-11 rounded-xl" iconClassName="size-5" />
                <div>
                  <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-muted-foreground">Planificación docente</p>
                  <h1 className="mt-1 text-xl font-black tracking-tight text-foreground sm:text-2xl">{entry.title}</h1>
                </div>
              </div>
              <div className="text-left text-xs leading-5 text-muted-foreground sm:text-right">
                <p className="font-extrabold text-foreground">{entry.schoolNameSnapshot || entry.schoolName || 'AulaBase'}</p>
            ...
            <div className="p-4 sm:p-6">
              ...
              <div className="overflow-x-auto border-x border-b border-border">
                <table className="w-full min-w-[1040px] table-fixed border-collapse text-left text-[11px] leading-[1.45] text-foreground">
                  <thead className="bg-primary/[0.07] text-[10px] uppercase tracking-[0.08em] text-foreground">
                    <tr>
...
function DocumentBand({ title, className = '' }: { title: string; className?: string }) {
  return <h2 className={`rounded-t-xl bg-warning/24 px-3 py-2 text-[11px] font-extrabold uppercase tracking-[0.16em] text-foreground ${className}`}>{title}</h2>
}
...
function CurriculumCell({ title, value }: { title: string; value?: string | null }) {
  return <section className="border-b border-r border-border p-3"><h3 className="text-[10px] font-extrabold uppercase tracking-[0.1em] text-primary-variant">{title}</h3><p className="mt-1.5 whitespace-pre-line text-[11px] leading-5 text-foreground">{value || 'Pendiente'}</p></section>
}
...
function SequenceRow({ day, entry }: { day: PlanningDay; entry: PlanningEntryWithDetails }) {
  return (
    <tr className="align-top odd:bg-card even:bg-muted/20">
      <td className="border-r border-t border-border p-2.5 font-bold"><span className="block text-primary-variant">Día {day.day}</span><span className="mt-1 block font-normal text-muted-foreground">{formatDate(day.date)}</span></td>
      ...
      <td className="border-r border-t border-border p-2.5"><strong className="text-primary-variant">Técnica</strong><p className="mt-1 whitespace-pre-line">{day.evaluationMethod || entry.evaluationMethod || 'Pendiente'}</p><strong className="mt-2 block text-primary-variant">Instrumento</strong><p className="mt-1 whitespace-pre-line">{day.evaluationInstruments || entry.evaluationInstruments || 'Pendiente'}</p></td>
...
function ActivityMoment({ label, value }: { label: string; value?: string | null }) {
  return <div className="mb-2 last:mb-0"><strong className="text-primary-variant">{label}:</strong><span className="ml-1 whitespace-pre-line">{value || 'Pendiente'}</span></div>
}
