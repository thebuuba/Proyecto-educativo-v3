...
import { SchoolSearchInput, type SchoolResult } from '@/modules/auth/components/SchoolSearchInput'
  ...
  return <div>
    ...
    <div className="mt-5 overflow-hidden rounded-2xl border border-border">
      ...
      <div className="grid gap-0 divide-y divide-border sm:grid-cols-2 sm:divide-x sm:divide-y-0">
        <div className="space-y-5 p-5"><SummaryItem label="Docente" value={draft.fullName} /><SummaryItem label="Año escolar" value={draft.schoolYear.name.replace('-', '–')} badge={draft.schoolYear.historical ? 'Histórico' : 'Actual'} /><SummaryItem label="Centro educativo" value={school.name} details={[school.district, school.centerCode ? `Código ${school.centerCode}` : null]} /></div>
