...
import { useGuidedTours } from './guidedTourContext'
        ...
        {availableTours.map((tour) => {
          ...
          return (
            <article key={tour.key} className="flex items-center gap-4 rounded-2xl border border-border p-4">
              ...
              <Button variant={complete ? 'outline' : 'primary'} onClick={() => {
                onClose()
                if (complete) replayTour(tour)
                else startTour(tour, resume ? saved.lastStep : 0)
              }}>
