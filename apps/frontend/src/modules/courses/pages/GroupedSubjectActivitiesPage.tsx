...
const blockVisuals: BlockVisual[] = [
  { icon: BookOpen, iconClass: 'bg-blue-50 text-blue-700 ring-blue-100', borderClass: 'border-blue-200 hover:border-blue-300', selectedClass: 'border-blue-400 bg-blue-50/70 ring-2 ring-blue-100', headerClass: 'bg-blue-50/55', lineClass: 'bg-blue-500' },
  { icon: Brain, iconClass: 'bg-emerald-50 text-emerald-700 ring-emerald-100', borderClass: 'border-emerald-200 hover:border-emerald-300', selectedClass: 'border-emerald-400 bg-emerald-50/70 ring-2 ring-emerald-100', headerClass: 'bg-emerald-50/55', lineClass: 'bg-emerald-500' },
  { icon: HandHeart, iconClass: 'bg-amber-50 text-amber-700 ring-amber-100', borderClass: 'border-amber-200 hover:border-amber-300', selectedClass: 'border-amber-400 bg-amber-50/70 ring-2 ring-amber-100', headerClass: 'bg-amber-50/55', lineClass: 'bg-amber-500' },
  { icon: FlaskConical, iconClass: 'bg-violet-50 text-violet-700 ring-violet-100', borderClass: 'border-violet-200 hover:border-violet-300', selectedClass: 'border-violet-400 bg-violet-50/70 ring-2 ring-violet-100', headerClass: 'bg-violet-50/55', lineClass: 'bg-violet-500' },
]
      ...
      <header className="w-full overflow-visible rounded-2xl bg-card shadow-sm">
        <div className="flex min-h-[76px] items-center gap-3 px-4 py-3 sm:px-5">
          <div className="flex min-w-0 flex-1 items-center gap-3">
            <button type="button" onClick={backToSubjects} className="flex h-10 shrink-0 items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white px-2.5 text-xs font-extrabold text-primary transition hover:border-primary/25 hover:bg-primary/[0.04]"><ArrowLeft className="size-4" /><span className="hidden sm:inline">Volver</span></button>
            ...
            <div className="min-w-0"><div className="flex min-w-0 flex-nowrap items-center gap-2 overflow-hidden"><h1 className="truncate text-base font-extrabold leading-tight text-foreground">{courseLabel} – {subjectName}</h1><span className="shrink-0 rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-extrabold text-emerald-700">Activa</span></div><p className="mt-1.5 flex min-w-0 flex-nowrap items-center gap-x-1.5 overflow-hidden whitespace-nowrap text-[11px] font-semibold text-muted-foreground">{levelName ? <span>{levelName.replace(/^nivel\s+/i, '')}</span> : null}{cycleName ? <><span>·</span><span>{cycleName}</span></> : null}<span>·</span><span>Sección {context.section.name}</span>{currentSchoolYear?.name ? <><span>·</span><span className="truncate">Año escolar {currentSchoolYear.name}</span></> : null}</p></div>
          </div>
...
function ActivitySavedModal({ activity, mode, onClose, onView, onEvaluate }: { activity: GradingActivity | null; mode: 'created' | 'updated'; onClose: () => void; onView: () => void; onEvaluate: () => void }) {
  return <Modal title={mode === 'updated' ? 'Actividad actualizada' : 'Actividad creada'} description={mode === 'updated' ? 'Los cambios se guardaron correctamente y ya se reflejan en AulaBase.' : 'La actividad se guardó correctamente y ya está disponible en la asignatura.'} icon={ClipboardCheck} tone="success" onClose={onClose} className="max-w-xl"><div className="p-5 sm:p-6"><section className="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-4"><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-emerald-700">Guardado correctamente</p><h3 className="mt-1 text-lg font-black text-foreground">{activity?.name ?? 'Actividad'}</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">Puedes volver a la lista, consultar el detalle completo o comenzar a calificarla.</p></section><div className="mt-5 grid gap-2 sm:grid-cols-3"><Button variant="outline" onClick={onClose}>Volver</Button><Button variant="outline" onClick={onView}><Eye className="size-4" /> Ver actividad</Button><Button onClick={onEvaluate}><ClipboardCheck className="size-4" /> Calificar</Button></div></div></Modal>
}

function ActivityStatusBadge({ status }: { status: SubjectActivityStatus }) {
  const styles = status === 'graded' ? 'bg-emerald-50 text-emerald-700' : status === 'partial' ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-600'
