  ...
  return <div className="space-y-3">
    ...
    <section className="overflow-hidden rounded-3xl bg-card shadow-sm">
      <div className="flex flex-col gap-4 border-b border-border px-5 py-5 lg:flex-row lg:items-center lg:justify-between">
        <div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-primary">Organización semanal</p><h2 className="mt-1 text-xl font-extrabold">Horario de la asignatura</h2><p className="mt-1 text-sm text-muted-foreground">Consulta cuándo impartes esta asignatura durante la semana.</p></div>
        <Link to="/horario"><Button variant="outline"><Pencil className="size-4" /> Editar horario</Button></Link>
      </div>

      {!sorted.length ? <div className="p-5"><EmptyState title="Sin horario" description="Todavía no hay clases registradas para esta asignatura. Puedes configurarlas desde el horario general." /></div> : <>
        <div className="grid gap-3 p-5 md:grid-cols-3">
          <SummaryCard icon={<CalendarDays className="size-5" />} label="Clases semanales" value={`${sorted.length}`} helper={`${new Set(sorted.map((entry) => entry.dayOfWeek)).size} días con clase`} />
          <SummaryCard icon={<Clock3 className="size-5" />} label="Tiempo semanal" value={`${weeklyMinutes} min`} helper={weeklyMinutes >= 60 ? `${Math.floor(weeklyMinutes / 60)} h ${weeklyMinutes % 60 ? `${weeklyMinutes % 60} min` : ''}`.trim() : 'Carga semanal'} />
          <SummaryCard icon={<CalendarDays className="size-5" />} label="Próxima clase" value={nextClass ? `${dayLabels[nextClass.entry.dayOfWeek]} · ${formatTime(nextClass.entry.startTime)}` : '—'} helper={nextClass?.offset === 0 ? 'Hoy' : nextClass ? 'Próximo encuentro' : 'Sin clases'} emphasis />
        </div>
        ...
        <div className="border-t border-border px-5 py-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{sorted.map((entry) => {
            const isToday = entry.dayOfWeek === today
            const duration = minutesBetween(entry.startTime, entry.endTime)
            return <article key={entry.id} className={cn('group relative overflow-hidden rounded-2xl border bg-card p-4 shadow-sm transition-all duration-200 motion-reduce:transform-none motion-reduce:transition-none hover:-translate-y-0.5 hover:shadow-md', isToday ? 'border-primary/30 ring-1 ring-primary/10' : 'border-border')}>
              <div className={cn('absolute inset-x-0 top-0 h-1', isToday ? 'bg-primary' : 'bg-primary/20')} />
              <div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-primary">{dayLabels[entry.dayOfWeek]}</p>{isToday ? <span className="rounded-full bg-primary/8 px-2 py-0.5 text-[9px] font-black uppercase text-primary">Hoy</span> : null}</div><p className="mt-2 text-xl font-black tracking-tight">{formatTime(entry.startTime)} – {formatTime(entry.endTime)}</p></div><span className="grid size-10 place-items-center rounded-xl bg-primary/[0.07] text-primary"><Clock3 className="size-5" /></span></div>
              <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-border pt-3 text-xs text-muted-foreground"><span className="inline-flex items-center gap-1.5"><Clock3 className="size-3.5" /> {duration} min</span>{entry.room ? <span className="inline-flex items-center gap-1.5"><MapPin className="size-3.5" /> {entry.room}</span> : null}</div>
            </article>
          })}</div>
        </div>
